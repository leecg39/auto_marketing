...
이 검증은 동의 허용, 회원가입, 로그인, 상품 조회, 장바구니, 결제 시작, 구매 완료, 리드 생성, 구매 중복 방지, dataLayer 개인정보 제거, CRM 플로우 매핑을 브라우저 DOM에서 확인합니다.
또한 웰컴 쿠폰, 고객 활동 갱신, 장바구니/결제 이탈, 구매 후 리뷰, 재구매, 광고 제외, 리드 후속, 60/90일 휴면 복귀, VIP 혜택 `automation_actions`가 생성되는지도 확인합니다.
