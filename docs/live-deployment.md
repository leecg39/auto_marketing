...
브라우저 SDK의 `crmWebhookUrl`은 자사몰의 same-origin 서버 route를 가리켜야 합니다. 이 route가 로그인 세션 또는 검증된 폼 요청을 확인하고, 고객 DB의 연락처와 저장된 수신동의를 payload에 다시 구성한 뒤 위 Bearer 토큰으로 `/api/crm/events`에 전달합니다. 브라우저가 보낸 이메일, 전화번호, `marketing_consent` 값만 신뢰해 발송하면 안 됩니다. 공개 데모처럼 인증된 고객 문맥이 없으면 연락처를 제외하고 이벤트·자동화 action 계약만 검증합니다.
